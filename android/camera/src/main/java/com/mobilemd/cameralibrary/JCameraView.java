...
//    private static final String TAG = "JCameraView";
    ...
    private void initView() {
        ...
        View childView = LayoutInflater.from(mContext).inflate(R.layout.camera_view, null);
        mVideoView = childView.findViewById(R.id.video_preview);
        mPhoto = childView.findViewById(R.id.image_photo);
        mCaptureLayout = childView.findViewById(R.id.capture_layout);
        mCaptureLayout.setDuration(60 * 1000);
        mFocusView = childView.findViewById(R.id.fouce_view);
        ...
        mCaptureLayout.setCaptureListener(new CaptureListener() {
            ...
            public boolean canTakePhoto() {
                return jCameraListener.imageIsMax();
            }

            @Override
            public boolean canRecorder() {
                return jCameraListener.videoIsMax();
            }
...
//        });
    ...
    public void changeCameraNum(int number){
        if(mCaptureLayout != null){
            mCaptureLayout.changeCameraNum(number);
        }
    }
...
//                FileUtil.deleteFile(videoUrl);
    ...
    @Override
    public void playVideo(final Bitmap firstFrame, final String url) {
        videoUrl = url;
        JCameraView.this.firstFrame = firstFrame;
        mPhoto.setImageBitmap(firstFrame);
        mPhoto.setVisibility(VISIBLE);
        mPhoto.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (jCameraListener != null) {
                    jCameraListener.recordSuccess(url, firstFrame);
                }
