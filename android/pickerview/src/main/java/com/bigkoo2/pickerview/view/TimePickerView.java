...
import com.bigkoo2.pickerview.configure.PickerOptions;
...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new WheelTime(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent,mPickerOptions.isIncomplete);
    ...
    private void initDefaultSelectedDate() {
        ...
        Log.i("GetOpsUrl","start:"+mPickerOptions.startDate);
        Log.i("GetOpsUrl","end:"+mPickerOptions.endDate);
    }
    ...
    private void setTime() {
        ...
        if (mPickerOptions.date != null) {
        ...
        }else if(!TextUtils.isEmpty(mPickerOptions.dateString)){
            ...
            String[] out = mPickerOptions.dateString.split(" ");
            if(out.length > 1){
                first = out[0];
                last = out[1];
            }else{
            ...
            if(first.split("-").length >= 3){
                t_year = first.split("-")[0];
                t_month = first.split("-")[1];
                t_day = first.split("-")[2];
            }
            Log.i("setTime0","last:"+last);
            ...
            if(TextUtils.isEmpty(t_hour)){
                t_hour = "00";
            }

            if(TextUtils.isEmpty(t_min)){
                t_min = "00";
            }

            if(TextUtils.isEmpty(t_sec)){
                t_sec = "00";
            }
            ...
            year = Integer.parseInt(t_year);
            month = Integer.parseInt(t_month)-1;
            day = Integer.parseInt(t_day);
            hours = Integer.parseInt(t_hour);
            minute = Integer.parseInt(t_min);
            seconds = Integer.parseInt(t_sec);

            Log.i("setTime1","t_year:"+t_year+" t_month:"+t_month+" t_day:"+t_day+" t_hour:"+t_hour+" t_min:"+t_min+" t_sec:"+t_sec);
        }else{
        ...
        Log.i("setTime2","year:"+year+" month:"+month+" day:"+day+" hour:"+hours+" minute:"+minute+" seconds:"+seconds);
    ...
    public void returnData() {
        ...
        }else if(mPickerOptions.incompleteTimeSelectListener != null){
            mPickerOptions.incompleteTimeSelectListener.onTimeSelect(wheelTime.getTime(), clickView);
        }
