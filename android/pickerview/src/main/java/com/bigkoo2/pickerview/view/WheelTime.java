package com.bigkoo2.pickerview.view;

import android.util.Log;
import android.view.View;

import com.bigkoo2.pickerview.R;
import com.bigkoo2.pickerview.adapter.ArrayWheelAdapter;
import com.bigkoo2.pickerview.adapter.NumericWheelAdapter;
import com.bigkoo2.pickerview.listener.ISelectTimeCallback;
import com.bigkoo2.pickerview.utils.ChinaDate;
import com.bigkoo2.pickerview.utils.LunarCalendar;
...
public class WheelTime {
    ...
    public WheelTime(View view, boolean[] type, int gravity, int textSize,boolean isIncomplete) {
        super();
        this.view = view;
        this.type = type;
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        // 年
        wv_year = (WheelView) view.findViewById(R.id.year);
        int maxYearInCompleted = Calendar.getInstance().get(Calendar.YEAR);
        wv_year.setAdapter(new NumericWheelAdapter(isIncomplete?1969:startYear, isIncomplete?maxYearInCompleted:endYear));// 设置"年"的显示数据


        wv_year.setCurrentItem(year - startYear +(isIncomplete?1:0));// 初始化时显示的数据
        wv_year.setGravity(gravity);
        // 月
        wv_month = (WheelView) view.findViewById(R.id.month);
        if (startYear == endYear) {//开始年等于终止年
            wv_month.setAdapter(new NumericWheelAdapter(isIncomplete?0:startMonth, endMonth));
            wv_month.setCurrentItem(month + (isIncomplete?2:1) - startMonth);
        } else if (year == startYear) {
            //起始日期的月份控制
            wv_month.setAdapter(new NumericWheelAdapter(isIncomplete?0:startMonth, 12));
            wv_month.setCurrentItem(month + (isIncomplete?2:1) - startMonth);
        } else if (year == endYear) {
            //终止日期的月份控制
            wv_month.setAdapter(new NumericWheelAdapter(isIncomplete?0:1, endMonth));
            wv_month.setCurrentItem(isIncomplete?(month+1):month);
        } else {
            wv_month.setAdapter(new NumericWheelAdapter(isIncomplete?0:1, 12));
            wv_month.setCurrentItem(isIncomplete?(month+1):month);
        }
        wv_month.setGravity(gravity);
        // 日
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startDay, isIncomplete?31:endDay));
            ...
            wv_day.setCurrentItem(day - startDay + (isIncomplete?1:0));
        } else if (year == startYear && month + 1 == startMonth) {
            // 起始日期的天数控制
            if (list_big.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startDay, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startDay, isIncomplete?31:30));
            } else {
                // 闰年 29，平年 28
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startDay, isIncomplete?31:(leapYear ? 29 : 28)));
            }
            wv_day.setCurrentItem(day - startDay + (isIncomplete?1:0));
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:1, isIncomplete?31:endDay));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
            ...
            wv_day.setCurrentItem(day - 1+(isIncomplete?1:0));
        } else {
            // 判断大小月及是否闰年,用来确定"日"的数据
            if (list_big.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:1, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:1, isIncomplete?31:30));
            } else {
                // 闰年 29，平年 28
                wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startDay, isIncomplete?31:(leapYear ? 29 : 28)));
            }
        ...
        //时
        wv_hours = (WheelView) view.findViewById(R.id.hour);
        wv_hours.setAdapter(new NumericWheelAdapter(isIncomplete?-1:0, 23));

        wv_hours.setCurrentItem(h+(isIncomplete?1:0));
        wv_hours.setGravity(gravity);
        //分
        wv_minutes = (WheelView) view.findViewById(R.id.min);
        wv_minutes.setAdapter(new NumericWheelAdapter(isIncomplete?-1:0, 59));

        wv_minutes.setCurrentItem(m+(isIncomplete?1:0));
        wv_minutes.setGravity(gravity);
        //秒
        wv_seconds = (WheelView) view.findViewById(R.id.second);
        wv_seconds.setAdapter(new NumericWheelAdapter(isIncomplete?-1:0, 59));

        wv_seconds.setCurrentItem(s+(isIncomplete?1:0));
        wv_seconds.setGravity(gravity);
        ...
        wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                Log.i("TimePickerView","month changed");
                if (startYear == endYear) {
    ...
    private void setReDay(int year_num, int monthNum, int startD, int endD, List<String> list_big, List<String> list_little) {
        ...
        monthNum = isIncomplete?monthNum-1:monthNum;
        if(isIncomplete && monthNum == 0){
            endD = 31;
            wv_day.setAdapter(new NumericWheelAdapter(isIncomplete?0:startD, endD));
...
//                maxItem = endD;
        ...
        if (currentYear == startYear) {
           /* int i = wv_month.getCurrentItem() + startMonth;
            ...
            if ((wv_month.getCurrentItem() + startMonth) == startMonth) {
                sb.append((wv_year.getCurrentItem() + startYear - (isIncomplete?1:0))).append("-")
                        .append((wv_month.getCurrentItem() + startMonth - (isIncomplete?1:0))).append("-")
                        .append((wv_day.getCurrentItem() + startDay - (isIncomplete?1:0))).append(" ")
                        .append(wv_hours.getCurrentItem() - (isIncomplete?1:0)).append(":")
                        .append(wv_minutes.getCurrentItem() - (isIncomplete?1:0)).append(":")
                        .append(wv_seconds.getCurrentItem() - (isIncomplete?1:0));
            } else {
                sb.append((wv_year.getCurrentItem() + startYear - (isIncomplete?1:0))).append("-")
                        .append((wv_month.getCurrentItem() + startMonth - (isIncomplete?1:0))).append("-")
                        .append((wv_day.getCurrentItem() + 1 - (isIncomplete?1:0))).append(" ")
                        .append(wv_hours.getCurrentItem() - (isIncomplete?1:0)).append(":")
                        .append(wv_minutes.getCurrentItem() - (isIncomplete?1:0)).append(":")
                        .append(wv_seconds.getCurrentItem() - (isIncomplete?1:0));
            }
            ...
            sb.append((wv_year.getCurrentItem() + startYear - (isIncomplete?1:0))).append("-")
                    .append((wv_month.getCurrentItem() + 1 - (isIncomplete?1:0))).append("-")
                    .append((wv_day.getCurrentItem() + 1 - (isIncomplete?1:0))).append(" ")
                    .append(wv_hours.getCurrentItem() - (isIncomplete?1:0)).append(":")
                    .append(wv_minutes.getCurrentItem() - (isIncomplete?1:0)).append(":")
                    .append(wv_seconds.getCurrentItem() - (isIncomplete?1:0));
    ...
    public void setRangDate(Calendar startDate, Calendar endDate) {
        Log.i("TimePickerView","setRangDate");
